...
use Thruster\Component\HttpMessage\MultipartStream;
...
class MultipartStreamTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $f1 = FnStream::decorate(stream_for('foo'), [
            'getMetadata' => function () {
                return '/foo/bar.txt';
            }
        ]);

        $f2 = FnStream::decorate(stream_for('baz'), [
            'getMetadata' => function () {
                return '/foo/baz.jpg';
            }
        ]);

        $f3 = FnStream::decorate(stream_for('bar'), [
            'getMetadata' => function () {
                return '/foo/bar.gif';
            }
        ]);
